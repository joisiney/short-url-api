...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private configService: ConfigService<Env, true>) {
    const dbHost = this.configService.get('PG_HOST', { infer: true });
    const dbPort = this.configService.get('PG_PORT', { infer: true });
    const dbUser = this.configService.get('PG_USER', { infer: true });
    const dbPassword = this.configService.get('PG_PWD', { infer: true });
    const dbName = this.configService.get('PG_NAME', { infer: true });
    const dbSsl = this.configService.get('PG_SSL', { infer: true });
    const poolMin = this.configService.get('PG_POOL_MIN', { infer: true });
    const poolMax = this.configService.get('PG_POOL_MAX', { infer: true });
    const connectionTimeoutMillis = this.configService.get(
      'PG_CONNECTION_TIMEOUT_MS',
      { infer: true },
    );
    const idleTimeoutMillis = this.configService.get('PG_IDLE_TIMEOUT_MS', {
      infer: true,
    });
